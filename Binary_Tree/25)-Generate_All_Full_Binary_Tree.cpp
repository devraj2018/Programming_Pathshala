...
This base case comes from Memorization (DP) : if we have seen/stored the vector of Full Binary Trees/roots already for some n, just 
-return that. I used a map for memorization.

Main Part/Recursive Part

Firstly get all the possible node combinations that exist in the left and right subtree. For example: for n = 7: 
the combinations can be (1,5);(3,3);(5,1). Use recursion to get all the possible combinations.
Now for all the combinations, find all the possible permutations using nested loops, and push them into ans vector. 
Finally return the vector, but remember to memorize it.*/
